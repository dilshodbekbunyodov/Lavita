<template>
  <div>
        ...
        <Listbox as="div" v-model="selectedUser">
          ...
          <div class="relative mt-2">
            ...
            <span class="flex items-center">
              <span class="block truncate">{{ selectedUser?.lastname }} {{ selectedUser?.username || 'Введите логин'
                }}</span>
            </span>
        ...
        <Listbox as="div" v-model="selectedWarehouse">
          ...
          <div class="relative mt-2">
            <ListboxButton
              class="relative flex w-full cursor-default rounded-md bg-white py-2 pl-3 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 sm:text-sm sm:leading-6">
            <span class="flex items-center">
              <span class="block truncate"> {{ selectedWarehouse?.title || 'Выберите склад' }}</span>
            </span>
              <span class="pointer-events-none absolute inset-y-0 right-0 ml-3 flex items-center pr-2">
              <ChevronUpDownIcon class="h-5 w-5 text-gray-400" aria-hidden="true" />
            </span>
    ...
    <paginate
      v-if="movements_count.total_pages"
      :page-count="movements_count.total_pages"
      :page-range="3"
      :margin-pages="2"
...
import tableMovement from '@/components/invoice/tableMovement.vue'
...
watchDebounced(() => [search.value.name, selectedUser.value, selectedWarehouse.value], async function() {
