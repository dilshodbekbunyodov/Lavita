...
<!--        <pre>{{warehouseList}}</pre>-->
        <tbody class="divide-y divide-gray-200 bg-white">
        <tr v-for="(product, inx5) in warehouseList[selected]" :key="inx5">
          <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">
            {{ product.name }}
          </td>
          <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
            {{product.count}}
<!--            <span v-if="selected === 'products'"> {{warehouseList.product_total_quantity}}</span>-->
<!--           <span  v-if="selected === 'parts'">   {{warehouseList.part_total_quantity}}</span>-->
<!--            <span v-if="selected === 'containers'">{{warehouseList.container_total_quantity}}</span>-->
          </td>
          <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
            <button
              type="button"
        ...
        <tr v-if="warehouseList && warehouseList[selected] && warehouseList[selected].length == 0">
...
watchDebounced(() => search.value.title, async function(title) {
  if (title.length > 0) {
