...
<!--<pre>{{movements}}</pre>-->
              ...
              <tr
                ...
                :key="index"
                @click="toRoute({ name: 'showmovement', params: { id: movement.id || '' } })"
              ...
              >
                ...
                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                  {{ movement?.warehouse_receiver_address }}
                </td>
                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                  {{movement.price ? movement.price.toFixed(1): ''}} сум
<!--                  {{movement.price ? movement.price.toLocaleString() : ''}} сум-->
<!--                  <br>-->
<!--                  {{ parseInt(movement.price) }} сум-->
                </td>
                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                  ...
                  <div class="text-gray-400">
                    {{ convertDate(movement?.updated_at, 'full') }}
                  </div>
                </td>
                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                    <span
                      :class="`px-2.5 py-0.5 rounded-[10px] flex-grow-0 flex-shrink-0 text-xs ${statusClass(movement.status)} font-medium text-center `"
                    >
                      {{ statusTitle(movement.status) }}
                    </span>
                </td>
