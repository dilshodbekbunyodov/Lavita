            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    <div
                      class="flex items-center gap-2 text-sm text-center px-[13px] py-[9px] text-gray-500"
                    >
                      <!-- {{ item }} -->
                      <input
                        type="checkbox"
                        v-model="item.checked"
                        @input="isChecked($event, item.id, index)"
                      />
                      {{ index + 1 }}
                    </div>
                    <input
                      v-model="item.id"
                      type="text"
                      class="block w-full rounded-r border-0 py-1.5 px-2.5 bg-white text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 focus:outline-0 sm:text-sm sm:leading-6"
                    />
                    <!--                    <div-->
                    <!--                      class="flex-1 rounded-r-lg px-[13px] py-[9px] bg-white flex-grow text-sm text-gray-900"-->
                    <!--                    >-->
...
const isChecked = (e:any, id:any, index) => {
...
const clear = () => {
  // model.value = {
  //   ...model.value,
  //   codes: model.value?.codes ? model.value?.codes?.map((c) => '') : []
  // }
  close()
}
